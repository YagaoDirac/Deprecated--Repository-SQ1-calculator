...
int LayerData::flip(LayerData& lowerLayer, int upper, int lower)
{
	if (!LayerData::upper)//dont confuse with param for this func.
		return -1;//-1 this layer is lower. Call this function through upper layer.
...
bool LayerData::operator==(const LayerData& a)
{
	...
	{ 
		if (IDs[i] != a.getID(i))
			return false;
		if (IDs[i] % 2 == 0 && bMainPart[i] != a.getbMainPart(i))
			return false;
	}
